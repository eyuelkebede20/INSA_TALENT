...
export async function assignPlayerToTeam(playerId, playerTier, tx = db) {
...
export async function checkAndLockTeam(teamId, tx = db) {
    const teamPlayers = await tx.execute(sql `SELECT tier FROM players WHERE team_id = ${teamId}`);
    const advCount = teamPlayers.filter((p) => p.tier === 'ADVANCED').length;
    const midCount = teamPlayers.filter((p) => p.tier === 'MID').length;
    const begCount = teamPlayers.filter((p) => p.tier === 'BEGINNER').length;

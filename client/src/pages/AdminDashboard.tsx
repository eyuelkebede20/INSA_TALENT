...
export default function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-base-200 overflow-hidden">
      ...
      <div className="absolute top-4 left-4 z-50 flex flex-col gap-4 w-72 pointer-events-auto">
        <div className="card bg-base-100/90 shadow-2xl border border-base-300 backdrop-blur-md">
      ...
      <div className="absolute bottom-4 right-4 z-50 w-80 h-96 pointer-events-auto flex flex-col">
        <div className="card bg-base-100/90 shadow-2xl border border-base-300 backdrop-blur-md h-full flex flex-col">
          <div className="p-3 border-b border-base-200 bg-base-100/50 rounded-t-2xl shrink-0 flex items-center justify-between">
                                    ...
                                    <Draggable key={m.id} draggableId={m.id} index={index}>
                                      {(provided, snapshot) => (
                                        <div
                                          ...
                                          className={`p-3 rounded-xl border flex justify-between items-center gap-2 transition-all ${snapshot.isDragging ? 'bg-primary/20 border-primary shadow-2xl rotate-2 z-50 scale-105' : 'bg-base-200 border-base-300 hover:border-primary/40'}`}
